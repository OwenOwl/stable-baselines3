...
class ExpertRolloutBuffer(RolloutBuffer):
    def __init__(
        ...
        if dataset_path.endswith(("pkl", "pickle")):
            ...
            if "oracle_state" not in h5_obs_dict.keys() or len(h5_obs_dict.keys()) > 1:
                raise RuntimeError(f"For non dict observation space, only oracle_state is allowed.")
            h5_obs = h5_obs_dict["oracle_state"]
            h5_action = h5_data["actions"]
            buffer_size = h5_action.shape[0]
        else:
            raise RuntimeError(f"Unrecognized file type for dataset path: {dataset_path}")
    ...
    def get(self, batch_size: Optional[int] = None, loop=True) -> Generator[RolloutBufferSamples, None, None]:
        ...
        while start_idx < self.buffer_size:
            yield self._get_samples(indices[start_idx: min(start_idx + batch_size, self.buffer_size)])
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> RolloutBufferSamples:
        ...
        return RolloutBufferSamples(*tuple(map(self.to_torch, data)), None, None, None, None)
