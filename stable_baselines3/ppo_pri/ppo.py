...
class Predictor(nn.Module):
    def __init__(self, o_dim, s_dim, encoder, tau=0.005):
        ...
        self.predictor = nn.Sequential(*create_mlp(input_dim=o_dim,
                                                  output_dim=s_dim,
                                                  net_arch=[256, 256]))
...
class PPO(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        buffer_cls = DictSSLRolloutBuffer if isinstance(self.observation_space, gym.spaces.Dict) else RolloutBuffer
        ...
        self.predictor = Predictor(self.features_dim,
                                   self.observation_space['state'].shape[0],
                                   self.policy.features_extractor).to(self.device)

        self.predictor_optimizer = th.optim.Adam(self.predictor.parameters(), lr=self.learning_rate)
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + self.rep_coef * pred_loss
