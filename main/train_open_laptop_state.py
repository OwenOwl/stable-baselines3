...
from hand_env_utils.teleop_open_laptop_env import create_open_laptop_env
...
if __name__ == '__main__':
    ...
    wandb_run = setup_wandb(config, exp_name, tags=["state", "open_latop", object_name])

    def create_env_fn():
        environment = create_open_laptop_env(object_name, use_visual_obs=False, object_category=object_cat,
                                             randomness_scale=randomness)
        return environment
    
    def create_eval_env_fn():
        environment = create_open_laptop_env(object_name, use_visual_obs=False, is_eval=True, object_category=object_cat,
                                             randomness_scale=randomness)
        return environment
