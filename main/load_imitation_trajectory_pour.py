...
def create_env(use_visual_obs, use_gui=True, obj_scale=1.0, obj_name="tomato_soup_can",
    ...
    env = ImitationPourEnv(**env_params)
...
def create_lab_env(use_visual_obs, use_gui=True, obj_scale=1.0, obj_name="tomato_soup_can",
    ...
    import os
    from hand_teleop.env.rl_env.free_pour_env import FreePourEnv
    from hand_teleop.real_world import task_setting
    from hand_teleop.env.sim_env.constructor import add_default_scene_light
    frame_skip = 5
...
if __name__ == '__main__':
    ...
    EMB_DIM = 32
    model_list_path = "/home/lixing/results/result-pour"
    ...
    for model_exp in model_list[:]:
        if model_exp != "pour-140-0.0,0.1,0.02":
            continue
        for iters in range(1):
            ...
            env = create_env(use_visual_obs=False, obj_scale=1, obj_name=(object_cat, object_name),
                            data_id=data_id, randomness_scale=randomness)
            lab_env = create_lab_env(use_visual_obs=False, obj_scale=1, obj_name=(object_cat, object_name),
            ...
            print("reward: ", lab_reward, reward)
            print(env.check_in_tank_particles())
            # if (lab_reward > 2):
