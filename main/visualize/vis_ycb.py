...
if __name__ == '__main__':
    ...
    for friction in [1]:
        for scale in [1]:
            succeed = 0
            for (object_name) in ["mustard_bottle"]:
                seed = 1
                save_str = "pics/"+exp+"_"+object_name+"_r"+str(seed)+"/"
                os.makedirs(save_str, exist_ok=True)
                randomness = 1.0

                lab_env = create_lab_env(use_visual_obs=False, obj_scale=scale, friction=friction, obj_name=object_name,
