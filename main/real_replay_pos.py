...
if __name__ == '__main__':
    ...
    print(env.control_time_step)
    print(env.frame_skip, env.scene.get_timestep())
    # exit()
    ...
    for action, obs in zip(traj, obs_traj):
        ...
        # exit()
        env.xarm.step_angle(
            obs["state"][:7], max_speed=20, wait=False
        )
        hand_qpos = obs["state"][7:]
        hand_qpos = np.concatenate([
            hand_qpos[:8:2],
            hand_qpos[8:10],
            # hand_qpos[8:9],
        ])
        print("Set Hand Angle: ", hand_qpos)
        env.ability_hand.set_joint_angle(hand_qpos)
        time.sleep(env.control_time_step * 20)

        robot_state = env.get_robot_state()
        print(robot_state)
        states_sequence.append(robot_state)
        print(obs["state"][:7] - prev_obs)
        prev_obs = obs["state"][:7]
    ...
    with open(f"{traj_root}/{traj_idx}/real_traj.pkl", "wb") as f:
        pickle.dump(states_sequence, f)
